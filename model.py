...
class Net:
    def __init__(self, inputs, labels, hidden_dim=50):
        ...
        W1 = tf.Variable(tf.truncated_normal([input_dim,hidden_dim],std=0.1))
        b1 = tf.Variable(tf.constant(0.1,shape=[hidden_dim])
        
        W2 = tf.Variable(tf.truncated_normal([hidden_dim,output_dim],std=0.1))
    ...
    def load_parameters(self, sess):
        ...
        if hasattr(self, "params_prev"):
            for i in range(len(self.params)):
                sess.run(self.parameters[i].assign(self.params_prev[i]))
    ...
    def compute_Fisher(self, data, sess, sample_size=200):
        ...
        self.Fisher = []
        for i in range(len(self.params)):
            self.Fisher.append(np.zeros(self.params[i].shape))
        ...
        for i in range(sample_size):
            ...
            # First derivative
            log_L_grad = sess.run(tf.gradients(tf.log(probs[0,class_ind]),self.params), feed_dict={self.x: data[i_data:i_data+1]})
            # Squared and added
            for i in range(len(self.Fisher)):
                self.Fisher[i] += np.square(log_L_grad)
